...
def sktch(imgfile):
  ...
  rknsketch = cv2.divide(gray_image, inverted_blurred_image,scale= 256.0)
  fname = imgfile.split('.')[0] + '_test1.jpg'
  cv2.imwrite(fname, rknsketch)
  return fname
...
left_col = [[sg.Text('Folder'), sg.In(size=(25,1), enable_events=True ,key='-FOLDER-'), sg.FolderBrowse()],
            ...
            [sg.Text('Resize to'), sg.In(default_text = "300",key='-W-', size=(5,1)), sg.In(default_text = "300", key='-H-', size=(5,1))]]

# For now will only show the name of the file that was chosen
images_col = [[sg.Text(size=(40,2), key='-TOUT-')],
              [sg.Image(key='-IMAGE-')],
              [sg.Button(button_text = "Convert", disabled = True,key='-Convert-'), sg.Exit()]]
...
while True:
    ...
    if event == '-FOLDER-':                         # Folder name was filled in, make a list of files in the folder
        ...
        try:
            ...
            sktchfile = sktch(filename)
            window['-TOUT-'].update(sktchfile)
            window['-IMAGE-'].update(data=convert_to_bytes(sktchfile, resize=new_size))
        except Exception as E:
            print(f'** Error {E} **')
            pass        # something weird happened making the full filename
